...
ava('display-morning(T)', (test): void => {
	...
	const formatted = timestamp.display(date);
	test.is(formatted, '4:20 AM');
});

ava('display-morning(t)', (test): void => {
	const timestamp = new Timestamp('t');
	const formatted = timestamp.display(date);
	test.is(formatted, '4:20:35 am');
});

ava('display-morning(LLL)', (test): void => {
	const timestamp = new Timestamp('LLL');
	const formatted = timestamp.display(date);
	test.is(formatted, 'March 09, 2019 4:20 AM');
});

ava('display-morning(lll)', (test): void => {
	const timestamp = new Timestamp('lll');
	const formatted = timestamp.display(date);
	test.is(formatted, 'Mar 09, 2019 4:20 AM');
});

ava('display-morning(LLLL)', (test): void => {
	const timestamp = new Timestamp('LLLL');
	const formatted = timestamp.display(date);
	test.is(formatted, 'Saturday, March 09, 2019 4:20 AM');
});

ava('display-morning(llll)', (test): void => {
	const timestamp = new Timestamp('llll');
	const formatted = timestamp.display(date);
	test.is(formatted, 'Sat Mar 09, 2019 4:20 AM');
});

ava('display-morning-number-overload(LLLL)', (test): void => {
	const timestamp = new Timestamp('LLLL');
	const formatted = timestamp.display(date.valueOf());
	test.is(formatted, 'Saturday, March 09, 2019 4:20 AM');
});

ava('display-morning-string-overload(LLLL)', (test): void => {
	const timestamp = new Timestamp('LLLL');
